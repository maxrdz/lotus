...
pub trait NavPageExt
where
    Self: Sized,
{
    fn title(&self) -> String;

    fn icon(&self) -> cosmic::widget::icon::Icon;

    fn all() -> &'static [Self];
}

impl NavPageExt for config::NavPage {
    fn title(&self) -> String {
        match self {
...
impl Application for Spacelink {
    ...
    fn init(core: Core, flags: Self::Flags) -> (Self, Task<Self::Message>) {
        ...
        for &nav_page in config::NavPage::all() {
            let id = nav
                .insert()
                .icon(nav_page.icon())
                .text(nav_page.title())
                .data::<config::NavPage>(nav_page)
                .id();
